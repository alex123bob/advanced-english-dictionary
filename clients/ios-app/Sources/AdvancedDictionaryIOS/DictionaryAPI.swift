...
struct DictionaryAPI {
    func lookup(word: String, apiBaseURL: String) async throws -> DictionaryEntry {
        ...
        guard let url = URL(string: "\(base)/api/dictionary") else { throw URLError(.badURL) }
        ...
        if decoded.success == false, let error = decoded.error { throw DictionaryAPIError.server(error) }
        return decoded
    }
}
